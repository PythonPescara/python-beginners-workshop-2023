...
class MediaPipeHand:
    def __init__(
        self, static_image_mode=True, max_num_hands=1, model_complexity=1, intrin=None
    ):
        ...
        for i in range(max_num_hands):
            p = {
                ...
                "rvec": np.zeros(
                    3
                ),  # Global rotation vector Note: this term is only used for solvepnp initialization
                "tvec": np.asarray(
                    [0, 0, 0.6]
                ),  # Global translation vector (m) Note: Init z direc to some +ve dist (i.e. in front of camera), to prevent solvepnp from wrongly estimating z as -ve
                "fps": -1,  # Frame per sec
    ...
    def result_to_param(self, result, img):
        ...
        if result.multi_hand_landmarks is not None:
            ...
            for i, res in enumerate(result.multi_handedness):
                if i > self.max_num_hands - 1:
                    break  # Note: Need to check if exceed max number of hand
            ...
            for i, res in enumerate(result.multi_hand_landmarks):
                ...
                for j, lm in enumerate(res.landmark):
                    self.param[i]["keypt"][j, 0] = (
                        lm.x * img_width
                    )  # Convert normalized coor to pixel [0,1] -> [0,width]
                    self.param[i]["keypt"][j, 1] = (
                        lm.y * img_height
                    )  # Convert normalized coor to pixel [0,1] -> [0,height]
        ...
        if result.multi_hand_world_landmarks is not None:
            for i, res in enumerate(result.multi_hand_world_landmarks):
                ...
                self.param[i]["angle"] = self.convert_joint_to_angle(
                    self.param[i]["joint"]
                )
    ...
    def convert_joint_to_camera_coor(self, param, intrin, use_solvepnp=True):
        ...
        if use_solvepnp:
            ...
            b[:, 0] = (
                -intrin["fx"] * param["joint"][idx, 0]
                + (param["keypt"][idx, 0] - intrin["cx"]) * param["joint"][idx, 2]
            )
            b[:, 1] = (
                -intrin["fy"] * param["joint"][idx, 1]
